...
A step by step workflow for how to run NNMM is shown in this section. The workflow below
...
Given the data and model equations, several different types of models are available in NNMM as shown below. In the table below, "X"
...
```
---
The `NNMM` package is loaded, as well as the `CSV` and `DataFrame` packages for reading text files.
