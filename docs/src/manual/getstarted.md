...
To load the NNMM package, use the following command inside the Julia REPL (or IJulia Notebook):

```julia
using NNMM
```

The command `Pkg.add("NNMM")` will add the registered official NNMM package and dependencies.

To use the latest/beta features under development, run `Pkg.add(PackageSpec(name="NNMM", rev="master"))` to get the
newest unofficial NNMM. Run `Pkg.free("NNMM")` to go back to the official one.
...
!!! note "Jupyter Notebooks with NNMM via Docker"

    Docker provides a straightforward way to install Jupyter Notebooks with NNMM.
...
token. Examples for NNMM can be accessed from the notebook: `notebooks/0_index.ipynb`.
...
Then, open the url in an internet browser (IE, Firefox, Chrome, Safari, etc) if NNMM-docker is launched on your local machine.
...
!!! note "standalone application (no installation required)"
    ...
    A fully self-contained application for NNMM (no installation required) will come out next year.
...
!!! warning
    ...
    Please load the NNMM package at first.

To show the basic information (README file) of NNMM in REPL or IJulia notebook using `?NNMM`
and press enter.
