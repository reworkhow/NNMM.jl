...
using DelimitedFiles,NNMM, Statistics, Distributions, DataFrames, LinearAlgebra,CSV
using Random
Random.seed!(314);
...
dam        = NNMM.mkmat_incidence_factor(data[!,:dam],[IDs;missing])*[dam_effect;0.0];

##non-genetic effects
x1_effect,x2_effect,x3_effect  = 1.0, randn(5), randn(2)
x1 = data[!,:x1]*x1_effect
x2 = NNMM.mkmat_incidence_factor(data[!,:x2],["g1","g2","g3","g4","g5"])*x2_effect
x3 = NNMM.mkmat_incidence_factor(data[!,:x3],["m","f"])*x3_effect;
