...
[![Build Status](https://travis-ci.com/reworkhow/NNMM.jl.svg?branch=master)](https://travis-ci.org/reworkhow/NNMM.jl)
...
NNMM.jl is an open-source software tool written in Julia for Bayesian multiple regression methods applied to genomic prediction and genome-wide association studies.

* **Homepage**: [QTL.rocks](https://QTL.rocks)
* **Discussion group**: [available here](https://groups.io/g/qtlrocks)
* **Installation**: at the Julia REPL, `using Pkg; Pkg.add("NNMM")`
...
3. Run `Pkg.add(PackageSpec(name="NNMM", rev="master"))` to get the newest unofficial NNMM. Run `Pkg.free("NNMM")` to go back to the official one.
